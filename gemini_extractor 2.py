...
class GeminiStructuredOutputError(RuntimeError):
    """Invalid structured output returned by Gemini."""
...
def _structured_call(
    ...
    *,
    ...
    try:
        ...
        is_truncated = (
            "eof while parsing" in error_text
            or "unexpected eof" in error_text
            or "unterminated string" in error_text
...
def _single_page_venue_jobs(
    doc: InputDocument,
    *,
    first: int | None,
    original_name: str,
) -> list[tuple[InputDocument, int, int, str]]:
    """Split a failed PDF batch into page-level jobs preserving page numbers."""
    parts = split_pdf(
        doc,
        pages_per_batch=1,
        start_page=1,
    ...
    page_offset = (first or 1) - 1
    jobs: list[tuple[InputDocument, int, int, str]] = []
    for part, relative_first, relative_last in parts:
        absolute_first = page_offset + relative_first
        absolute_last = page_offset + relative_last
        part.name = (
            f"{Path(original_name).stem}_"
            f"p{absolute_first}-{absolute_last}.pdf"
        )
        jobs.append(
            (
                part,
...
def _venue_job_instruction(
    *,
    ...
    if compact:
        instruction += (
            " MODO DE RECUPERAÇÃO: devolva JSON válido e conciso. "
            "Não repita textos integrais, URLs ou rodapés. Limite evidence "
            "a um trecho curto de até 300 caracteres e mantenha descrições, "
            "listas, inclusões, exclusões e restrições objetivas. "
            "Não use Markdown fora do JSON."
        )
    return instruction


def _failed_venue_batch_warning(
    *,
    original_name: str,
    first: int | None,
    last: int | None,
    ...
    page_text = (
        f"página {first}"
        if first == last and first is not None
        else f"páginas {first} a {last}"
        if first is not None and last is not None
        else "lote sem página identificada"
    )
    ...
    return VenueBatch(
        ...
        venues=[],
        warnings=[
            "A NAVE preservou os demais resultados, mas não conseguiu "
            f"extrair {original_name}, {page_text}. Motivo: {reason}"
        ],
    )
...
def extract_venues(
    ...
    *,
    ...
    while job_index < len(jobs):
        doc, first, last, original_name = jobs[job_index]
        if progress_callback:
            progress_callback(
                completed,
                total,
        ...
        instruction = _venue_job_instruction(
        ...
        try:
            ...
            if can_split:
                page_jobs = _single_page_venue_jobs(
                    doc,
                    first=first,
                    original_name=original_name,
                )
                jobs[job_index:job_index + 1] = page_jobs
                total += len(page_jobs) - 1
                if progress_callback:
                    progress_callback(
                        completed,
                        total,
                        (
                            "O lote devolveu uma resposta incompleta. "
                            f"A NAVE dividiu {original_name}, páginas "
                            f"{first} a {last}, em páginas individuais."
                        ),
                    )
                continue

            # A single page can still contain long footer/source text. Retry
            # once with strict compact-output instructions before isolating it.
            compact_instruction = _venue_job_instruction(
                original_name=original_name,
                first=first,
                last=last,
                compact=True,
            ...
            try:
                batch = _structured_call(
                    ...
                    docs=[doc],
                    schema=VenueBatch,
                    context=f"{doc.name} — recuperação compacta",
                )
            except GeminiStructuredOutputError as retry_exc:
                batch = _failed_venue_batch_warning(
                    original_name=original_name,
                    first=first,
                    last=last,
                    exc=retry_exc,
