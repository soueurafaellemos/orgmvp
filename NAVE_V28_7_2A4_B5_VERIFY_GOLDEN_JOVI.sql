-- NAVE by VOE · V28.7.2A4+B5 — Golden JOVI identity/strategy precision verification
...
current_outcomes as (
  select eco.* from public.entity_current_outcomes eco join tp on tp.project_id=eco.project_id
),
core_relations as (
...
checks as (
  select jsonb_build_object(
    ...
    'four_platform_solutions_are_evidence_led',(
      select count(*)=4
      from solutions
      where (lower(name) like '%youtube%' or lower(name) like '%instagram%' or lower(name) like '%tiktok%' or lower(name) like '%kwai%')
        and coalesce(attributes->>'origin','')='evidence_led_v2872a'
    ),
    'platform_proposal_truths_verified',(
      select count(*)>=4
      from current_outcomes eco
      join solutions s on s.entity_id=eco.entity_id
      where eco.outcome_type='proposal_status'
        and eco.outcome_status='proposed'
        and (lower(s.name) like '%youtube%' or lower(s.name) like '%instagram%' or lower(s.name) like '%tiktok%' or lower(s.name) like '%kwai%')
    ),
    'exactly_four_platform_solution_identities',(
      select count(*)=4
      from solutions
      where lower(name) like '%youtube%'
         or lower(name) like '%instagram%'
         or lower(name) like '%tiktok%'
         or lower(name) like '%kwai%'
    ),
    'exactly_four_current_platform_proposal_truths',(
      select count(*)=4 and count(distinct eco.entity_id)=4
      from current_outcomes eco
      join solutions s on s.entity_id=eco.entity_id
      where eco.outcome_type='proposal_status'
        and eco.outcome_status='proposed'
    ...
    'a_reconciliation_converged_for_platforms',coalesce((
      select observations_total=7
         and observations_reconciled=6
         and observations_open=1
         and evidence_reconciled_solutions=4
         and evidence_led_created_solutions=4
      from a_status
    ),false),
    'strategy_count_is_7',(select count(*)=7 from strategy),
    'no_cross_platform_occurrence_collapse',not exists(
      select 1
      from public.project_solution_occurrences pso
      join solutions s on s.id=pso.solution_instance_id
      join tp on tp.project_id=pso.project_id
      where pso.lifecycle_status='active'
        and lower(trim(coalesce(pso.observed_name,''))) ~ ' activation$'
        and lower(trim(coalesce(s.name,''))) ~ ' activation$'
        and regexp_replace(lower(trim(pso.observed_name)), '\s+activation\s*$', '') <>
            regexp_replace(lower(trim(s.name)), '\s+activation\s*$', '')
    ),
    'exactly_one_event_journey_architecture',(
      select count(*)=1 and bool_and(lower(name)='event journey')
      from experience
    ),
    'no_generic_journey_architecture',not exists(
      select 1 from experience where lower(trim(name)) in ('journey','jornada')
    ),
    'journey_unique_core_moments',(
      select count(distinct moment_type)=5
      from journey
      where moment_type in ('pre_event','event','post_event','product_reveal','activation_reveal')
    ),
    'no_meta_strategy_pollution',not exists(
      select 1 from strategy
      where (strategy_type='territory' and lower(trim(title)) in ('highlights','insight','insights'))
         or lower(title) like 'frequentadores de festivais%'
         or lower(title) like 'diretrizes de ativação por plataforma%'
    ),
    ...
    'no_unsupported_core_truth',not exists(
      ...
      where t.lifecycle_status='active'
        and t.truth_state in ('unsupported','review_required')
    ),
...
select
  ...
  (select to_jsonb(x) from a_status x) as reconciliation_status_a,
  coalesce((select jsonb_agg(to_jsonb(x) order by x.outcome_type,x.entity_id) from current_outcomes x),'[]'::jsonb) as current_outcomes,
