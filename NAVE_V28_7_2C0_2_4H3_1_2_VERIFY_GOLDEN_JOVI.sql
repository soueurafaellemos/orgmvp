-- NAVE by VOE · V28.7.2C0.2.4H3.1.2
...
-- "Requirement Truth Repair · V28.7.2C0.2.4H3.1.2"
-- no projeto JOVI X300.
--
-- H3.1.2 corrige dois falsos verdes possíveis do H3:
  ...
  select jsonb_build_object(
    ...
    'c024h312_run_completed', exists(
      ...
      where status='completed'
        and pipeline_version='V28.7.2C0.2.4H3.1.2'
    ),
    ...
    -- Critical H3.1.2 precision checks. Terminal punctuation is normalized first.
    ...
    'h312_run_declares_local_directive_guard', coalesce((
      select coalesce((metadata->>'local_directive_guard')::boolean,false)=true
      from req_run
    ),false),
    'h312_run_declares_base_h3_semantic_precedence', coalesce((
      select coalesce((metadata->>'base_h3_semantic_precedence')::boolean,false)=true
      from req_run
    ),false),
