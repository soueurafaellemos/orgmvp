-- NAVE by VOE · V28.7.2C0.2.4H3.1.3
...
-- "Requirement Truth Repair · V28.7.2C0.2.4H3.1.3"
-- no projeto JOVI X300.
--
-- H3.1.3 corrige dois falsos verdes possíveis do H3:
...
checks as (
  select jsonb_build_object(
    ...
    'c024h312_run_completed', exists(
      select 1 from req_run
      where status='completed'
        and pipeline_version='V28.7.2C0.2.4H3.1.3'
    ),
    ...
    -- Critical H3.1.3 precision checks. Terminal punctuation is normalized first.
    ...
    'instagram_creative_direction_remains_current_requirement', exists(
      select 1 from rt
      where truth_state in ('verified','human_confirmed')
        and normalized_terminal_title = regexp_replace(
          lower(trim('Direcionamento criativo: Construir um espaço altamente instagramável, indo além dos clichês tradicionais.')),
          '[[:space:][:punct:]]+$','', 'g'
        )
    ),
    'instagram_creative_direction_observation_is_requirement', exists(
      select 1 from obs
      where coalesce(attributes->>'origin_route','')='legacy_recall'
        and normalized_terminal_name = regexp_replace(
          lower(trim('Direcionamento criativo: Construir um espaço altamente instagramável, indo além dos clichês tradicionais.')),
          '[[:space:][:punct:]]+$','', 'g'
        )
        and semantic_role='requirement_candidate'
        and status='reconciled'
    ),
    'instagram_activation_heading_keeps_solution_reference_semantics', exists(
      select 1 from obs
      where coalesce(attributes->>'origin_route','')='legacy_recall'
        and normalized_terminal_name = regexp_replace(
          lower(trim('Ativação Instagram: Aesthetics & Lifestyle Gallery')),
          '[[:space:][:punct:]]+$','', 'g'
        )
        and semantic_role='solution_reference'
        and status='no_domain_object'
    ),

    -- H3.1.3 structural-role integrity: no-domain is not enough; the child must attach
    -- to the correct semantic container from the source structure.
    'youtube_product_children_are_product_attributes',
      (select count(distinct normalized_terminal_name)=3 from obs
       where coalesce(attributes->>'origin_route','')='legacy_recall'
         and normalized_terminal_name in (
           'jovi x300 ultra',
    ...
    'youtube_platform_children_are_platform_scope',
      (select count(distinct normalized_terminal_name)=4 from obs
       where coalesce(attributes->>'origin_route','')='legacy_recall'
         and normalized_terminal_name in (
    ...
    'instagram_platform_children_are_platform_scope',
      (select count(distinct normalized_terminal_name)=5 from obs
       where coalesce(attributes->>'origin_route','')='legacy_recall'
         and normalized_terminal_name in (
    ...
    'h313_run_declares_local_directive_guard', coalesce((
      select coalesce((metadata->>'local_directive_guard')::boolean,false)=true
      from req_run
    ),false),
    'h313_run_declares_structural_refinement_precedence', coalesce((
      select coalesce((metadata->>'structural_refinement_precedence')::boolean,false)=true
      from req_run
    ),false),
