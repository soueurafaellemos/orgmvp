    ...
    if repair_projects:
        ...
        if st.button(
            ...
            if outcome.get("errors") or outcome.get("incomplete"):
                ...
                st.success(f"{outcome.get('processed', 0)} arquivo(s) reprocessado(s) com a leitura especializada da V28.3.0.")
            ...
            if cross and str(cross.get("status") or "").startswith("completed"):
                with st.expander("Intelligence Graph · conexões entre arquivos", expanded=False):
                    ...
                    st.caption(
                        "A V28.3.0 consolida evidências, relatório pós-evento, conexões cross-source e uma verdade única do projeto. "
