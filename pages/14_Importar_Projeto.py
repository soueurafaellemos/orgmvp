...
page_header(
    ...
    eyebrow="NAVE by VOE · V28.7.2C0.2.4H3",
)
...
def _requirement_reconciliation_result(container: dict) -> dict:
    direct = container.get("requirement_reconciliation") if isinstance(container, dict) else None
    if isinstance(direct, dict):
        return direct
    finalization = container.get("project_intelligence_finalization") if isinstance(container, dict) else None
    if isinstance(finalization, dict) and isinstance(finalization.get("requirement_reconciliation"), dict):
        return finalization["requirement_reconciliation"]
    return {}


def _render_requirement_reconciliation(container: dict, *, expanded: bool = False) -> None:
    result = _requirement_reconciliation_result(container)
    if not result:
        return
    status = str(result.get("status") or "")
    if status == "schema_missing":
        st.warning(
            "Requirement Semantic Reconciliation V28.7.2C0.2.4H3 ainda não está visível no Data API. "
            "O H3 reutiliza o contrato SQL C0.2.4 já instalado. Se o schema estiver ausente, execute NAVE_V28_7_2C0_2_4H1_RESOLUTION_ACTION_CONTRACT_HOTFIX.sql no Supabase e rode novamente."
        )
        return
    if status in {"schema_check_error", "transaction_error", "orchestration_error", "blocked"} or status.startswith("blocked_"):
        st.error(
            "A V28.7.2C0.2.4H3 não promoveu uma nova geração de Requirement Reconciliation. "
        ...
        return
    if status not in {"completed", "semantic_gate_blocked"}:
        st.warning(f"Requirement Semantic Reconciliation V28.7.2C0.2.4H3: {status or 'status desconhecido'}.")
        return

    rec = result.get("requirement_reconciliation") or {}
    semantic_gate = result.get("semantic_gate") or {}
    gate_blocked = status == "semantic_gate_blocked" or semantic_gate.get("pass") is False
    if gate_blocked:
        st.error(
            "Requirement Semantic Gate BLOCK: existem observações/identidades ainda não resolvidas. "
            "A V28.7.2B foi bloqueada nesta run; o estado anterior permanece válido em legacy_shadow."
        )
    actions = result.get("actions") or {}
    with st.expander("Requirement Semantic Reconciliation · V28.7.2C0.2.4H3", expanded=expanded):
        cols = st.columns(7)
        ...
        cols[5].metric("Observation review", int(rec.get("observations_review_required") or 0))
        ...
        role_cols[0].metric("Suggestion", int(rec.get("classified_suggestion") or 0))
        role_cols[1].metric("Example", int(rec.get("classified_example") or 0))
        role_cols[2].metric("Parameter", int(rec.get("classified_parameter") or 0))
        role_cols[3].metric("Constraint qualifier", int(rec.get("classified_constraint_qualifier") or 0))
        role_cols[4].metric("Semantic Gate", "PASS" if not gate_blocked else "BLOCK")

        st.caption(
            f"Migration mode: {rec.get('migration_mode') or 'legacy_shadow'} · "
            f"Domain schema: {rec.get('domain_schema_version') or '28.7.2c0.2.4'} · Run: {result.get('run_id') or rec.get('last_completed_run_id') or '—'}"
        )
        st.caption(
            "Requirement Identity ≠ Requirement Occurrence ≠ Constraint. Legacy requirement é apenas sinal de recall; "
            "Evidence histórica é provenance, não passe semântico. C0.2.4H3 classifica role antes de binding, preserva targets e rótulos nominais de produto/modelo como no-domain, preserva exemplos sem virar Requirement, mantém modalidade obrigatória mesmo quando a frase contém um verbo de sugestão e faz fail-closed quando restam observações semânticas em review/open. "
        ...
        suggestions = [str(v) for v in (actions.get("suggestions") or []) if str(v).strip()]
        examples = [str(v) for v in (actions.get("examples") or []) if str(v).strip()]
        parameters = [str(v) for v in (actions.get("parameters") or []) if str(v).strip()]
        constraint_qualifiers = [str(v) for v in (actions.get("constraint_qualifiers") or []) if str(v).strip()]
        ...
        if scopes or attributes or contexts or references or suggestions or examples or parameters or constraint_qualifiers or evidence_first or new_titles or diagnostics:
            with st.expander("Requirement Reconciliation · decisões desta run", expanded=False):
                if new_titles:
                    st.markdown("**Novos Requirements evidence-led**")
                    st.caption(" · ".join(new_titles))
                if scopes:
                    st.markdown("**Signals preservados como scope / canal**")
                    st.caption(" · ".join(scopes))
                if attributes:
                    st.markdown("**Signals preservados como atributo**")
                ...
                if suggestions:
                    st.markdown("**Sugestões / opcionais preservados sem virar Requirement**")
                    st.caption(" · ".join(suggestions))
                if examples:
                    st.markdown("**Exemplos preservados sem virar Requirement**")
                    st.caption(" · ".join(examples))
                if parameters:
                    st.markdown("**Parâmetros vinculáveis preservados**")
                    st.caption(" · ".join(parameters))
                if constraint_qualifiers:
                    st.markdown("**Qualificadores de constraint preservados**")
                    st.caption(" · ".join(constraint_qualifiers))
                if evidence_first:
                ...
                if unresolved:
                    ...
                    for row in unresolved[:20]:
                        st.caption("• " + str(row.get("title") or row.get("requirement_id") or "Requirement"))
...
with st.expander("Corrigir um projeto importado por uma versão anterior da V28", expanded=False):
    ...
    if repair_projects:
        ...
        if st.button(
            ...
            help="Executa Truth Gate + V28.7.2A + audits, reconcilia Requirement Identity/Occurrence em V28.7.2C0.2.4H3 por legacy recall + Evidence-first e só então materializa V28.7.2B. Não reconstrói o Graph V28.6.",
        ):
            from project_intelligence_pipeline import finalize_project_intelligence

            with st.spinner("Aplicando Truth Gate, reconciliando Solutions e Requirements e materializando Strategy / Creative / Experience sem reconstruir o Graph legado..."):
            ...
            _render_requirement_reconciliation(finalization, expanded=True)
            _render_core_semantics(finalization, expanded=False)
            domain = _domain_result(finalization)
            reconciliation = _reconciliation_result(finalization)
            requirement_reconciliation = _requirement_reconciliation_result(finalization)
            ...
            requirement_ok = str(requirement_reconciliation.get("status") or "") == "completed"
            core_ok = str(core_semantics.get("status") or "") == "completed"
            if domain_ok and reconciliation_ok and audits_ok and requirement_ok and core_ok:
                coverage = audits.get("coverage") or {}
                identity = audits.get("identity") or {}
                st.success(
                    "V28.7.2C0.2.4H3 materializada em legacy_shadow. Evidence → Observation → Domain agora inclui "
                ...
                req_status = str(requirement_reconciliation.get("status") or "")
                if req_status == "semantic_gate_blocked":
                    st.error(
                        "Truth Gate, Solution reconciliation e audits permaneceram válidos, mas o Requirement Semantic Gate bloqueou esta run. "
                        "A V28.7.2B não foi executada; nenhum cutover foi promovido e o Graph V28.6 permaneceu congelado."
                    )
                else:
                    st.error(
                        "Truth Gate, Solution reconciliation e audits permaneceram válidos, mas a Requirement Semantic Reconciliation V28.7.2C0.2.4H3 não terminou. "
