...
st.set_page_config(
    page_title="NAVE by VOE | Briefing e Recomendações",
...
page_header(
    ...
    "Histórico de briefings, diagnósticos, versões e "
    "recomendações do projeto selecionado.",
)
...
if projects.empty:
    st.info(
        "Este projeto ainda não possui briefing ou "
...
if (
    focused_project_id
    and not project_page.empty
):
    selected_project = (
        project_page.iloc[0]
        .to_dict()
    )
elif selected_project_rows:
    selected_project = (
        project_page.iloc[
