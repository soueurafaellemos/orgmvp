...
-- Golden JOVI — Bare Product Identity Guard Hotfix + Requirement Precision Verification
...
checks as (
  select jsonb_build_object(
    ...
    'c024h3_run_completed', exists(select 1 from req_run where status='completed' and pipeline_version='V28.7.2C0.2.4H3'),
    ...
    'jovi_bare_model_legacy_recall_is_no_domain', exists(
      ...
      where coalesce(attributes->>'origin_route','')='legacy_recall'
        and lower(trim(observed_name))='jovi x300 ultra'
    ...
    'hands_on_lab_not_bound_to_bare_product_identity', not exists(
      ...
      where coalesce(o.attributes->>'origin_route','')='evidence_first'
        ...
        and lower(trim(pr.title))='jovi x300 ultra'
