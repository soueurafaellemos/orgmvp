...
-- Incremental SQL. Preserves all existing status-view columns in place and only appends
-- role precision / fail-closed semantic-gate columns. No destructive DDL.
...
select
  ...
  '28.7.2c0.2.4'::text as domain_schema_version,
  ...
  -- C0.2.4 role-precision columns appended; existing view column order is preserved.
  (select count(*)::integer from public.semantic_observations so
    where so.project_id=p.id and so.domain_hint='requirement' and so.status <> 'superseded' and so.semantic_role='suggestion_signal'
  ) as classified_suggestion,
  (select count(*)::integer from public.semantic_observations so
    where so.project_id=p.id and so.domain_hint='requirement' and so.status <> 'superseded' and so.semantic_role='example_signal'
  ) as classified_example,
  (select count(*)::integer from public.semantic_observations so
    where so.project_id=p.id and so.domain_hint='requirement' and so.status <> 'superseded' and so.semantic_role='parameter_signal'
  ) as classified_parameter,
  (select count(*)::integer from public.semantic_observations so
    where so.project_id=p.id and so.domain_hint='requirement' and so.status <> 'superseded' and so.semantic_role='constraint_qualifier'
  ) as classified_constraint_qualifier,
  (
    (select count(*)::integer from public.semantic_observations so where so.project_id=p.id and so.domain_hint='requirement' and so.status='open') +
    (select count(*)::integer from public.semantic_observations so where so.project_id=p.id and so.domain_hint='requirement' and so.status='review_required') +
    (select count(*)::integer from public.project_requirement_truth_status t where t.project_id=p.id and t.truth_state='legacy_unverified' and t.legacy_explanation_role is null) +
    (select count(*)::integer from public.project_requirement_truth_status t where t.project_id=p.id and t.truth_state='conflicted') +
    (select count(*)::integer from public.project_requirement_truth_status t where t.project_id=p.id and t.truth_state='review_required')
  ) as semantic_gate_blockers,
  (
    (select count(*) from public.semantic_observations so where so.project_id=p.id and so.domain_hint='requirement' and so.status in ('open','review_required')) = 0
    and (select count(*) from public.project_requirement_truth_status t where t.project_id=p.id and t.truth_state='legacy_unverified' and t.legacy_explanation_role is null) = 0
    and (select count(*) from public.project_requirement_truth_status t where t.project_id=p.id and t.truth_state in ('conflicted','review_required')) = 0
  ) as semantic_gate_pass
...
select
  'V28.7.2C0.2.4 installed'::text as status,
