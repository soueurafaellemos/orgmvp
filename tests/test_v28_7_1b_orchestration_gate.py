...
def test_pipeline_blocks_downstream_when_domain_is_unavailable_or_not_completed():
    ...
    assert '"cross_source": {"status": "skipped_domain_blocked"}' in code
    assert 'if domain_status != "completed":' in code
    assert "probe_domain_schema" in code
    # The schema preflight must appear before report analysis in the function.
...
def test_ui_green_success_requires_domain_and_cross_source_success():
    ...
    assert "if domain_ok and cross_ok:" in code
    assert "elif not domain_ok:" in code
    assert "analyze_pending_reports=False" in code


def test_hotfix_b_sql_self_checks_and_reloads_postgrest_schema_cache():
