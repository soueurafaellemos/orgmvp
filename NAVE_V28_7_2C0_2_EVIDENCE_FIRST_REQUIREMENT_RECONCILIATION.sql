-- NAVE by VOE · V28.7.2C0.2 — Evidence-First Requirement Reconciliation
-- Incremental patch over V28.7.2C0 + C0.1.
--
-- Fixes three structural defects proven by Golden JOVI:
--   1) provenance != semantic identity: already-bound legacy rows no longer bypass C0;
--   2) recall != legacy inventory: current briefing Evidence can create missing Requirement candidates;
--   3) occurrence atomicity: stale C0 occurrences/observations are superseded, never deleted.
--
-- No Graph V28.6 rebuild. No domain_primary. No destructive DELETE.

begin;

-- --------------------------------------------------------------------------
-- 1. Requirement Truth Gate: current semantic occurrence / Human Review wins.
--    Historical domain_object_evidence remains auditable but cannot, alone,
--    verify a Requirement identity rejected by the current semantic gate.
-- --------------------------------------------------------------------------
...
with base as (
  ...
  select
    ...
    lo.attributes->>'origin_route' as legacy_explanation_origin_route
  from base b
  left join lateral (
...
-- --------------------------------------------------------------------------
-- 2. Debug/status view distinguishes legacy recall from current Requirement truth.
-- --------------------------------------------------------------------------
...
select
  ...
  (select count(*)::integer from public.project_requirement_truth_status t where t.project_id=p.id and t.legacy_source_id is not null and t.truth_state <> 'historical') as legacy_requirement_rows,
  (select count(*)::integer from public.project_requirement_truth_status t where t.project_id=p.id and t.truth_state in ('verified','human_confirmed','review_required')) as current_requirement_identities,
  ...
  (
    select count(*)::integer from public.semantic_observations so
    where so.project_id=p.id and so.domain_hint='requirement' and so.status <> 'superseded'
      and so.semantic_role in ('audience_context','strategy_context')
  ...
  '28.7.2c0.2'::text as domain_schema_version,
...
-- --------------------------------------------------------------------------
-- 3. Atomic writer C0.2. Same RPC name to keep orchestration stable.
-- --------------------------------------------------------------------------
...
begin
  if p_project_id is null or p_run_id is null then
    raise exception 'V28.7.2C0.2 requires project_id and run_id';
  end if;
  ...
  if v_project_entity_id is null then
    raise exception 'V28.7.2C0.2 project knowledge_entity missing';
  end if;
  ...
  if jsonb_array_length(coalesce(p_bundle->'observations','[]'::jsonb)) = 0
    ...
    raise exception 'V28.7.2C0.2 empty observation bundle blocked';
  end if;
  ...
  loop
    ...
    insert into public.knowledge_entities(
      ...
      jsonb_build_object('projection_only',true,'normalized_by','V28.7.2C0.2','origin','evidence_led_v2872c0_2'),
    ...
    insert into public.domain_object_governance(
      ...
      p_run_id,jsonb_build_object('normalized_by','V28.7.2C0.2','source_layer','evidence_led')
    )
...
comment on view public.project_requirement_truth_status is
  'V28.7.2C0.2 — Requirement Truth Gate. Historical Evidence link is provenance, not semantic identity. Current occurrence or Human Review decides current Requirement truth.';
comment on view public.project_requirement_reconciliation_status is
  'V28.7.2C0.2 — Requirement debugger separating legacy recall, evidence-first discovery, no-domain semantic roles and current Requirement truth.';
comment on function public.apply_project_requirement_reconciliation_v2872c0(uuid,uuid,jsonb) is
  'V28.7.2C0.2 — atomic evidence-first Requirement reconciliation. Stale C0 observations/occurrences are superseded, never deleted; no existing-existing auto-merge.';
...
select
  'V28.7.2C0.2 installed'::text as status,
  to_regprocedure('public.apply_project_requirement_reconciliation_v2872c0(uuid,uuid,jsonb)') is not null as rpc_ok,
  pg_catalog.encode(pg_catalog.sha256(pg_catalog.convert_to('NAVE_V28_7_2C0_2'::text,'UTF8')),'hex') as native_sha256_smoke_test;
