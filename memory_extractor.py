...
SECTION_KEYWORDS: dict[str, list[tuple[str, int]]] = {
    ...
    "activations": [
        ...
        ("mecanica", 5),
        ("activation", 10),
        ("platform activation", 12),
        ("youtube", 4),
        ("instagram", 4),
        ("tiktok", 4),
        ("kwai", 4),
        ("masterclass", 7),
        ("high resolution studio", 8),
        ("lifestyle gallery", 8),
        ("movement challenge", 8),
        ("backstage", 6),
        ("night mode", 5),
        ("hands on", 6),
    ],
    ...
    "communication": [
        ...
        ("paleta", 5),
        ("grafismo", 5),
        ("communications timeline", 14),
        ("save the date", 10),
        ("online invitation", 10),
        ("reminder", 8),
        ("invitation", 8),
    ],
    "content_agenda": [
...
CANDIDATE_PATTERNS: list[tuple[str, str, str]] = [
    # Padrões GENÉRICOS de live marketing. Não use nomes de clientes, venues,
    # fornecedores ou soluções de projetos específicos neste core.
    (r"\bevent journey\b|\bjornada (?:do|de) evento\b", "Jornada do evento", "journey_operation"),
    (r"\bcommunications? timeline\b|\bsave the date\b|\bonline invitation\b|\breminder\b", "Comunicação pré-evento", "communication"),
    (r"\bpress ?kit\b", "Press kit", "gifts"),
    (r"\bpassport holder\b|\bporta passaporte\b", "Porta-passaporte", "gifts"),
    (r"\bluggage tag\b|\btag de bagagem\b", "Tag de bagagem", "gifts"),
    (r"\bholographic bag\b|\bbolsa holografica\b", "Bag / bolsa", "gifts"),
    (r"\bcreator recommendation\b|\binfluencer recommendation\b", "Recomendação de creator", "partners_sponsorship"),
    (r"\bproduct reveal\b|\brevelacao do produto\b", "Product reveal", "content_agenda"),
    (r"\bwelcome experience\b|\bexperiencia de boas vindas\b", "Welcome experience", "journey_operation"),
    (r"\barrival tunnel\b|\btunel de chegada\b|\btunel receptivo\b", "Túnel de chegada", "scenography"),
    (r"\b(?:youtube|instagram|tiktok|kwai) activation\b|\bativacao (?:youtube|instagram|tiktok|kwai)\b", "Ativação de plataforma", "activations"),
    (r"\bphoto[\s-]?op\b|\bponto de foto\b|\bphoto opportunity\b", "Ponto de foto", "activations"),
    (r"\bquick massage\b|\bilha de massagem\b|\bmassagem express\b", "Quick Massage", "activations"),
    ...
    (r"\bm[uú]sica ao vivo\b|\bshow musical\b|\bdj set\b", "Atração musical", "content_agenda"),
    (r"\bpalestrante(?:s)?\b|\bmestre de cerim[oô]nias\b|\bkeynote\b", "Conteúdo / palestrantes", "content_agenda"),
    (r"\bcamarote\b", "Camarote", "scenography"),
    (r"\bestande\b|\bstand\b", "Estande", "scenography"),
    (r"(?im)(?:^|\n)\s*palco(?:\s+h\s*=|\s*[-–—:]|\s*$)|\bpalco\s+h\s*=", "Palco", "scenography"),
    ...
    (r"\bidentidade visual\b|\bvisual identity\b", "Identidade visual", "communication"),
    (r"\benvelopamento\b", "Envelopamento", "communication"),
    (r"\bsinaliza[cç][aã]o\b|\bsignage\b", "Sinalização", "communication"),
    (r"\bcredenciamento\b|\bcheck[- ]?in\b", "Credenciamento", "journey_operation"),
    (r"\bcircuito\b", "Circuito da experiência", "journey_operation"),
    (r"\bfluxo\b|\bflow\b", "Fluxo da experiência", "journey_operation"),
