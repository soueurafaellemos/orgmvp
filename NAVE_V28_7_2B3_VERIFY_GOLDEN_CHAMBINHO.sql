-- NAVE by VOE · V28.7.2B3 — Golden Chambinho semantic-scope + atomicity verification
...
checks as (
  select jsonb_build_object(
    ...
    'core_status_unsupported_zero',coalesce((
      select unsupported=0
      from public.project_core_semantic_status s join tp on tp.project_id=s.project_id
    ),false),
    'reconciliation_observation_scope_preserved_22',coalesce((
      select observations_total=22
         and observations_reconciled=20
         and observations_no_domain_object=2
      from public.project_domain_reconciliation_status s join tp on tp.project_id=s.project_id
    ),false),
