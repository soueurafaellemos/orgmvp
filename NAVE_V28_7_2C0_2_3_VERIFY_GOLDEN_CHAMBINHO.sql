-- NAVE by VOE · V28.7.2C0.2.3 — Golden Chambinho verification
...
with target_project as (
  ...
  select jsonb_build_object(
    ...
    'c023_schema_visible',coalesce((select domain_schema_version='28.7.2c0.2.3' from rs),false),
    'c023_run_completed',exists(select 1 from latest where analyzer_type='project_requirement_reconciliation' and status='completed' and pipeline_version='V28.7.2C0.2.3'),
    ...
    'legacy_recall_has_no_null_legacy_identity',not exists(
      select 1 from obs
      where coalesce(attributes->>'origin_route','')='legacy_recall'
        and nullif(attributes->>'legacy_requirement_id','') is null
    ),
    ...
    'active_c0_occurrences_are_c022',not exists(
      select 1 from occ where coalesce(attributes->>'normalized_by','') <> 'V28.7.2C0.2.3'
    ),
    ...
    'evidence_led_payment_requirement_is_verified',exists(
      select 1 from rt t
      where t.truth_state='verified'
        and t.legacy_source_id is null
        and lower(coalesce(t.title,'')) like 'precisamos organizar para pagarem a cenografia%'
    ),
    ...
    'graph_v28_6_not_rerun_after_c023',coalesce(
